    ...
    for P in ["PyQt5>=5.15.9", "numpy", "scipy>=1.11.4", "matplotlib>=3.6.0", "h5py", "Pillow", "lxml", "pyqtgraph>=0.13.7", "scikit-image>=0.19.1", "xdrlib3"]:  # copy list from ../setup.py
        ...
        try:
            ...
            if len(p) > 1:
                ...
                if op_str == "<":
                    check = parse_version(v_c) < parse_version(p[-1])
                elif op_str == ">":
                    check = parse_version(v_c) > parse_version(p[-1])
                elif op_str in ["=", "=="]:
                    check = parse_version(v_c) == parse_version(p[-1])
...
class PDFExporter(Exporter):
    ...
    def export(self, filename=None):
        ...
        pw.setPageSize(  # Tested with pyqt5-15.5.6
            QPageSize(QSizeF(self.getTargetRect().size()) / dpi * 25.4, QPageSize.Millimeter)
        )
        painter = QPainter(pw)
        try:
            self.setExportMode(True, {"antialias": True, "background": self.background, "painter": painter})
            painter.setRenderHint(QPainter.Antialiasing, True)
            if QtCore.QT_VERSION >= 0x050D00:
                painter.setRenderHint(QPainter.LosslessImageRendering, True)
            self.getScene().render(painter, QRectF(self.getTargetRect()), QRectF(self.getSourceRect()))
        finally:
