...
class data:
    def __init__(self, data_struct):
        ...
        self.shifts = []
        self.stack4D = None
        self.ask_fix_callback = None # Callback function(msg) -> bool

    # ----------------------------------------------------------------------
    # ... (skipping new_data and others)
    ...
    def i0_from_histogram(self, i0_indices):
        ...
        if self.stack4D is None:
            ...
            if self.calculate_optical_density() is False:
                return False
    ...
    #----------------------------------------------------------------------
    def handle_duplicates(self, a):
        if len(a) != len(np.unique(a)):
            # Identify duplicates and shift them
            u, indices, counts = np.unique(a, return_index=True, return_counts=True)
            duplicates = u[counts > 1]
            
            for d in duplicates:
                mask = (a == d)
                # mask has multiple Trues. leave first one alone, shift others.
                # indices where mask is True
                locs = np.where(mask)[0]
                for i in range(1, len(locs)):
                    a[locs[i]] += 1e-5 * i # Shift by small epsilon
                    print("Shifted duplicate energy at index {} from {} to {}".format(locs[i], d, a[locs[i]]))
        return a

    #----------------------------------------------------------------------
    def sort_by_indices(self, indices):
        """
        Sorts the data (ev, absdata, etc.) according to the provided int array.
        """
        if self.ev is not None and hasattr(self.ev, '__len__') and len(self.ev) == len(indices):
            self.ev = self.ev[indices]
        
        # Sort absdata (n_cols, n_rows, n_ev)
        if hasattr(self, 'absdata') and isinstance(self.absdata, np.ndarray):
            if self.absdata.ndim == 3 and self.absdata.shape[2] == len(indices):
                self.absdata = self.absdata[:, :, indices]
            elif self.absdata.ndim == 2: 
                pass # Probably inappropriate to sort? or (pixels, ev)?
        
        # Sort stack4D (x, y, ev, theta)
        if hasattr(self, 'stack4D') and isinstance(self.stack4D, np.ndarray):
             if self.stack4D.shape[2] == len(indices):
                 self.stack4D = self.stack4D[:,:,indices,:]

        # Sort od (n_pixels, n_ev)
        if hasattr(self, 'od') and isinstance(self.od, np.ndarray):
             if self.od.ndim == 2 and self.od.shape[1] == len(indices):
                 self.od = self.od[:, indices]
        
        # Sort od3d (n_cols, n_rows, n_ev)
        if hasattr(self, 'od3d') and isinstance(self.od3d, np.ndarray):
             if self.od3d.ndim == 3 and self.od3d.shape[2] == len(indices):
                 self.od3d = self.od3d[:, :, indices]
    ...
    def calculate_optical_density(self):
        ...
        # Check for duplicates in evi0 and fix them
        if len(self.evi0) != len(np.unique(self.evi0)):
            fix = True
            if self.ask_fix_callback:
                fix = self.ask_fix_callback("Duplicate energies found in I0 data.")
            
            if fix:
                self.evi0 = self.handle_duplicates(self.evi0)
            else:
                self.od = 0 # Reset to safe state (integer 0 as initialized)
                return False
